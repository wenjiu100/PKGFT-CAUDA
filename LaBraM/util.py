...
import datetime
import numpy as np
from pyhealth.metrics.prediction_set import miscoverage_ps
...
def prepare_SEED_dataset(root):
    ...
    train_files = os.listdir(os.path.join(root, "processed_train"))
    np.random.shuffle(train_files)
    val_files = os.listdir(os.path.join(root, "processed_val"))
    test_files = os.listdir(os.path.join(root, "processed_test"))

    print(len(train_files), len(val_files), len(test_files))

    # prepare training and test data loader
    train_dataset = SEEDLoader(os.path.join(root, "processed_train"), train_files)
    test_dataset = SEEDLoader(os.path.join(root, "processed_test"), test_files)
    val_dataset = SEEDLoader(os.path.join(root, "processed_val"), val_files)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset
