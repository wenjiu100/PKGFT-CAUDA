...
class Attention(nn.Module):
    def __init__(
        ...
        self.tuning_mode = tuning_mode
        if tuning_mode == 'ssf':
            # 为 qkv 变换后的特征 (dim*3) 初始化 SSF 参数
            self.ssf_scale_qkv, self.ssf_shift_qkv = init_ssf_scale_shift(all_head_dim * 3)
            # 为 proj 投影后的特征 (dim) 初始化 SSF 参数
            #self.ssf_scale_q, self.ssf_shift_q = init_ssf_scale_shift(head_dim)
            # self.ssf_scale_k, self.ssf_shift_k = init_ssf_scale_shift(head_dim)
            #self.ssf_scale_v, self.ssf_shift_v = init_ssf_scale_shift(head_dim)
            # self.ssf_scale_vo, self.ssf_shift_vo = init_ssf_scale_shift(head_dim)
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        ...
        if self.k_norm is not None:
            ...
            # ✅ 新增：分别对 q, k, v 应用 SSF（仅当 tuning_mode == 'ssf' 时）
        #if self.tuning_mode == 'ssf':
            # 注意：ssf_scale_q 等 shape 应为 (head_dim,)
            #q = q * self.ssf_scale_q + self.ssf_shift_q
            # k = k * self.ssf_scale_k + self.ssf_shift_k
           # v = v * self.ssf_scale_v + self.ssf_shift_v
        ...
        # ✅ 插入 SSF on VO (value output per head)
        # if self.tuning_mode == 'ssf':
        #     x = x * self.ssf_scale_vo + self.ssf_shift_vo  # 自动广播 (head_dim,) → (B,H,N,head_dim)
