...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    input_chans = None
    ortho_reg_mask = (torch.ones((12, 12)).cuda() - torch.eye(12).cuda()).float()
...
def evaluate(
    ...
    metrics=['acc'],
    is_binary=True,
    return_attention=False,   # 🔥 默认 False（关键）
):
    import torch
    from einops import rearrange
    import utils
    ...
    criterion = (
        ...
        if is_binary else torch.nn.CrossEntropyLoss()
    )
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            cls_feat, attentions, m_act = model.forward_features_with_metrics(
        ...
        # ===== metrics =====
        output_cpu = torch.sigmoid(output).cpu() if is_binary else output.cpu()
        ...
        true.append(target_cpu)

        metric_logger.update(loss=loss.item())
        for k, v in results.items():
            metric_logger.meters[k].update(v, n=EEG.size(0))

        # ===== attention (仅在需要时收集) =====
        if return_attention:
            all_m_act.append(m_act.cpu())   # [L]

            attentions = torch.stack(attentions, dim=0)  # [L, B, H, N, N]
            cls_attn = attentions[:, :, :, :, 0].mean(dim=-1)  # [L, B, H]
            all_cls_attentions.append(cls_attn.cpu())
    ...
    stats = utils.get_metrics(pred, true, metrics, is_binary, 0.5)
    ...
    # ===== M-Act′ =====
    all_m_act = torch.stack(all_m_act, dim=0)      # [num_batches, L]
    M_Act = all_m_act.mean(dim=0).numpy()          # [L]

    # ===== F-Attn =====
    all_cls_attentions = torch.cat(all_cls_attentions, dim=1)  # [L, total_B, H]
    F_Attn = all_cls_attentions.mean(dim=(1, 2)).numpy()       # [L]

    return stats, M_Act, F_Attn   # ✅ test 阶段
