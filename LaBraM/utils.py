...
def Entropy(input_):
    eps = 1e-5
    return -(input_ * torch.log(input_ + eps)).sum(1).mean()
...
def auto_load_model(
    ...
    """
    Auto resume model / optimizer / scaler according to tuning mode.

    Rules:
    - Always load model weights if checkpoint exists
    - Resume optimizer & scaler ONLY when tuning_mode != 'full'
    - Skip optimizer resume safely when param groups mismatch
    """
    ...
    if not getattr(args, 'enable_deepspeed', False):
        ...
        if args.auto_resume and len(args.resume) == 0:
            ...
            if os.path.exists(ckpt):
                ...
                all_ckpts = glob.glob(os.path.join(output_dir, 'checkpoint-*.pth'))
                latest = -1
                for c in all_ckpts:
                    t = c.split('-')[-1].split('.')[0]
                    if t.isdigit():
                        latest = max(latest, int(t))
                if latest >= 0:
                    args.resume = os.path.join(output_dir, f'checkpoint-{latest}.pth')

            if args.resume:
                print(f"[AutoResume] Found checkpoint: {args.resume}")
        ...
        if args.resume:
            ...
            # =================================================
            # 1. Load model weights (ALWAYS)
            # =================================================
            missing, unexpected = model_without_ddp.load_state_dict(
                checkpoint['model'], strict=False
            )
            print(f"[Resume] Model loaded from {args.resume}")
            if len(missing) > 0:
            ...
            # =================================================
            # 2. Decide whether to resume optimizer
            # =================================================
            resume_optimizer = (
                'optimizer' in checkpoint and
                'epoch' in checkpoint and
                args.tuning_mode != 'full'
            )
            ...
            if resume_optimizer:
                try:
                    ...
                    print(f"[Resume] Optimizer resumed at epoch {checkpoint['epoch']}")

                    if 'scaler' in checkpoint and loss_scaler is not None:
                        loss_scaler.load_state_dict(checkpoint['scaler'])
                        print("[Resume] Loss scaler resumed")

                except ValueError as e:
                    print(f"[WARN] Skip optimizer resume due to mismatch: {e}")
                    args.start_epoch = 0
            else:
                print(f"[Resume] Skip optimizer resume (tuning_mode = {args.tuning_mode})")
                args.start_epoch = 0
            ...
            if (
                ...
                hasattr(args, 'model_ema') and
                args.model_ema and
                'model_ema' in checkpoint
            ):
                _load_checkpoint_for_ema(model_ema, checkpoint['model_ema'])
                print("[Resume] EMA model resumed")

            # =================================================
            # 4. Discriminator optimizer (optional)
            # =================================================
            if optimizer_disc is not None and 'optimizer_disc' in checkpoint:
                try:
                    optimizer_disc.load_state_dict(checkpoint['optimizer_disc'])
                    print("[Resume] Discriminator optimizer resumed")
                except ValueError as e:
                    print(f"[WARN] Skip optimizer_disc resume: {e}")
    ...
    # =========================================================
        ...
        if args.auto_resume:
            all_ckpts = glob.glob(os.path.join(output_dir, 'checkpoint-*'))
            latest = -1
            for c in all_ckpts:
                t = c.split('-')[-1]
                if t.isdigit():
            ...
            if latest >= 0:
                tag = f'checkpoint-{latest}'
                print(f"[AutoResume] Deepspeed checkpoint: {tag}")
                _, client_states = model.load_checkpoint(args.output_dir, tag=tag)
                args.start_epoch = client_states.get('epoch', -1) + 1

                if (
                    model_ema is not None and
                    hasattr(args, 'model_ema') and
                    args.model_ema and
                    'model_ema' in client_states
                ):
                    _load_checkpoint_for_ema(model_ema, client_states['model_ema'])
...
#     # 获取训练集、验证集和测试集的文件路径
#     train_files = [os.path.join(root, "train", file) for file in os.listdir(os.path.join(root, "train"))]
#     np.random.shuffle(train_files)
#     val_files = [os.path.join(root, "val", file) for file in os.listdir(os.path.join(root, "val"))]
#     test_files = [os.path.join(root, "test", file) for file in os.listdir(os.path.join(root, "test"))]
...
#     # 初始化数据集
#     train_dataset = SEEDLoader(train_files)
#     test_dataset = SEEDLoader(test_files)
#     val_dataset = SEEDLoader(val_files)
...
def prepare_SEED_dataset(public_folder_path, test_folder_full_path):
    # 获取所有被试文件夹（公共文件夹下的子文件夹）
    all_subfolders = [
        os.path.join(public_folder_path, f)
        for f in os.listdir(public_folder_path)
        if os.path.isdir(os.path.join(public_folder_path, f))
    ]
    
    # 确保测试集文件夹路径有效
    assert os.path.exists(test_folder_full_path), "Test folder path does not exist"

    # 过滤训练集文件夹（排除测试集文件夹）
    train_folders = [f for f in all_subfolders if f != test_folder_full_path]

    # === 加载测试集 ===
    test_files = os.listdir(test_folder_full_path)
    test_dataset = SEEDLoader(
        root=test_folder_full_path,
        files=test_files
    )
    ...
    for folder in train_folders:
        ...
        train_subdataset = SEEDLoader(
            root=folder,
            files=train_files
        )
        train_datasets.append(train_subdataset)
...
#     # prepare training and test data loader
#     train_dataset = SLEEPLoader(os.path.join(root, "train"), train_files)
#     test_dataset = SLEEPLoader(os.path.join(root, "test"), test_files)
#     val_dataset = SLEEPLoader(os.path.join(root, "val"), val_files)
#     print(len(train_files), len(val_files), len(test_files))
#     return train_dataset, test_dataset, val_dataset
